...
/**
 * This class creates TextBox objects for drawing onto JPanel
 * It implements the MyShape interface
 * @author Zach
 *
 */

class MyTextBox implements MyText
{
	private int X, Y, W, H;
	...
	public MyTextBox(int startX, int startY, int w, int h)
	{
		X = startX;
		Y = startY;
		H = h;
		text = new String("Edit Text");
		font = new Font("Times New Roman", Font.PLAIN, H);
		color = ColorPanel.getCurrentColor();
		highlighted = false;
	}
	
	// Alternate constructor which allows a different text String to be used
	public MyTextBox(int startX, int startY, int h, String s,Color c)
	{
		X = startX;
		Y = startY;
		H = h;
	...
	{
		return ("G:" + X + ":" + Y + ":" + H + ":" + text + ":"+color.getRed()+":"+color.getGreen()+":"+color.getBlue());
	}
